...
(models trained through 30 Nov 2025, evaluated Dec 2025–Jan 2026): MAE 0.158 (0–24 h) / 0.174 (24–48 h)
...
| Канонический прогон после пересборки моделей: 28/28 трасс `mode=no-llm` | ✅ сохранён | [`forecasts/trace_*.json`](forecasts/), [происхождение](docs/INTEGRATION.md); отчёты шаблонные, прежние 28 LLM-выпусков — в git-истории `4e6f7ce` |
| Строгая репетиция macOS/Linux aarch64 glibc без сети, допуск `1e-9` | ✅ проверена | [`docs/REPRODUCIBILITY.md`](docs/REPRODUCIBILITY.md); GitHub Actions пока заблокирован биллингом организации |
...
максимальное отклонение прогнозов — 0, оценки — `1.11e-16`. Все 145 тестов прошли за 17.05 / 17.58 с.
`compare_runs` остаётся диагностикой; его код 0 сам по себе не гарантирует совпадение.
...
прогоняет модели обеих турбин, валидирует границы, полноту и NaN; flatline записывается как предупреждение,
сравнивает прогноз мощности с выпуском 09.02,
...
шесть узлов, все `ok`, `mode=no-llm`, `completed=true`, `fallback=false`. Отчёт шаблонный.
...
- **LLM управляет инструментами и составляет отчёт** при включённом LLM-режиме. Исторические 28 запусков `mode=openai` без fallback сохранены в [4e6f7ce](docs/INTEGRATION.md#канонические-llm-отчёты-4e6f7ce); GPT-6 Luna указан автором, точный model ID не записан. Новые 28 канонических выпусков выполнены с `--no-llm`: старые тексты не описывают пересобранные прогнозы.
...
- Строгая проверка — `python -m scripts.reproduce --output-dir runs/<имя>`: сверка прогнозов и оценки с допуском `1e-9`, сеть заблокирована, канонические файлы не меняются.
...
- На ретроспективной оценке первого дня MAE составляет 15.8 % номинала; сравнение с baseline и маска допустимых целей приведены ниже.
...
<details>
<summary><b>🖼️ Неделя ретроспективной оценки: прогноз и факт</b></summary>

![Неделя сохранённой ретроспективной оценки: прогноз и факт, lead 1](docs/img/holdout_week.png)

Построено из сохранённого `evaluation_predictions.csv` (модели обучены по 30.11.2025, lead 1, семь дней
с 12.01.2026); каждый день — отдельный выпуск, поэтому сегменты разрываются на границах выпусков.
Финальная модель не пересчитывается (`python -m scripts.plot_holdout`).
...
flowchart LR
    ...
    subgraph DayGraph["DayGraph · src/agent/graph.py · один граф для LLM и --no-llm"]
        ...
        C --> D{"validate_forecast<br/>границы, полнота, NaN<br/>flatline: предупреждение"}
...
Два режима: «Тестовый период (февраль 2026)» читает `forecasts/` или `--forecast-dir`; «Ретроспективная
оценка» читает сохранённые прогнозы `models_artifacts/evaluation/` — финальная модель не пересчитывается
...
В Linux aarch64 glibc прогон с отключённой сетью прошёл при допуске `1e-9`, как и на macOS;
обучение при сборке отсутствует. GitHub Actions ожидает разблокировки биллинга организации
([результаты](docs/REPRODUCIBILITY.md)).
...
```text
.
├── README.md · README.ru.md · README.kk.md   ← английский (по умолчанию), русский, казахский
...
| [`docs/DASHBOARD.md`](docs/DASHBOARD.md) | Режимы панели и источники данных |
...
- Linux aarch64 glibc проверен локально без сети с допуском `1e-9`. Hosted GitHub Actions не стартует
  из-за блокировки биллинга организации; локальный результат и состояние CI описаны отдельно
  в [`REPRODUCIBILITY`](docs/REPRODUCIBILITY.md).
...
- **LLM в продукте**: модель по умолчанию `gpt-6-luna` (OpenAI Chat Completions, tool calling); адаптеры Claude и NVIDIA NIM. Точный model ID исторических LLM-запусков `4e6f7ce` не записан; текущие канонические отчёты — `--no-llm`.
...
- **Графики** — SVG в `docs/img/`, генерируются скриптом из сохранённых CSV/JSON (не из памяти); PNG недели оценки — `scripts/plot_holdout.py` из сохранённого `evaluation_predictions.csv`.
...
| 23.09.2026 | Исторический LLM-прогон `4e6f7ce`: 28/28 трасс `mode=openai`; GPT-6 Luna указан автором; прежний комплект сохранён в git-истории |
| 23.09.2026 | Панель показывает сохранённую ретроспективную оценку вместо пересчёта модели; 145 тестов |
| 23.09.2026 | Признаки округляются до 10 знаков; обе группы моделей и прогнозы пересобраны; строгий macOS/Linux replay `1e-9`, 28 новых отчётов `--no-llm` |
