...
Нужен Python 3.11+. Данные организаторов и кэш всех погодных запросов уже в репозитории —
...
# 3. Независимая проверка подачи: полнота 1344 часов, границы [0,1], горизонты,
...
```
Агент (Claude / NVIDIA NIM / --no-llm)
  ├── fetch_weather      → Open-Meteo Previous Runs API, 5 NWP-источников (+кэш в репо)
  ├── prepare_features   → 108 фич: ветер 10–120 м, v³, плотность воздуха, сдвиг ветра,
  │                        направление, лаги/окна, согласие ансамбля, календарь, lead time
  ├── run_model          → LightGBM x5 сидов (бэггинг) + изотоническая кривая мощности
  ├── validate_forecast  → границы [0,1], полнота горизонта, NaN, flatline
  ├── compare_with_previous → дрейф против вчерашнего прогноза, решение о пересчёте
  └── write_outputs      → CSV + отчёт оператора
```
...
- Простои и ограничения мощности в феврале непредсказуемы по погоде — модель прогнозирует
  доступную ветровую выработку.
- Готовые продолжения: квантильный прогноз P10/P90 (LightGBM quantile), автоматическое
  переобучение при дрейфе NWP, интеграция с рынком балансирующей энергии — агентная
  архитектура допускает это без переделки ядра.
