...
Режим `--no-llm` исполняет **объявленный граф состояний** (`src/agent/graph.py`).
У графа есть цикл: неудачная валидация возвращает управление на повторный срез
закэшированного прогноза. Каждый переход пишется в трассу
`forecasts/trace_<дата>.json`, которую показывает панель оператора.
LLM-режим вызывает те же Python-тулы через отдельный цикл в `src/agent/loop.py`.
...
LLM-режим использует OpenAI GPT-6 Luna по умолчанию; доступны Claude и NVIDIA NIM.
Его вызовы тулов пока не записываются в трассу графа.
