...
Готовая матрица `build_features` округляется до 10 знаков после запятой одинаково
при обучении и прогнозе. Это предотвращает разделение математически равных признаков
порогами деревьев из-за различий `libm`. Изменение точности требует переобучения обоих
комплектов моделей; доказательство переносимости описано в [REPRODUCIBILITY](REPRODUCIBILITY.md).
...
| `app.py` | Streamlit: сохранённая оценка, прогнозы, отчёты и трассы |
| `scripts/reproduce.py` | Offline-проверка полного выпуска, оценки и SHA256 канонических файлов |
...
`--evaluation-dir` задаёт отдельный каталог сохранённой ретроспективной оценки.
