...
| Канондық LLM іске қосуы: 28/28 трасса, `mode=openai`, `fallback=false`; GPT-6 Luna коммит авторымен көрсетілген | ✅ сақталған | [`forecasts/trace_*.json`](forecasts/), [шығу тегі](docs/INTEGRATION.md#канонические-llm-отчёты-4e6f7ce); трассаларда model ID жоқ |
...
| Оператор панелі (Streamlit) · тесттер `pytest tests/ -q` | ✅ 71 passed | `streamlit run app.py`, [`tests/`](tests/) |
| Linux-та релиз репетициясы (таза клоннан Docker) | 🔄 жеке тексеру | [`docs/NEXT_STEPS.md`](docs/NEXT_STEPS.md) |
| P10–P90 аралықтарын калибрлеу (қамту 65 %, номинал 80 %) | ⬜ ашық тапсырма | [`evaluation_report.json`](models_artifacts/evaluation/evaluation_report.json) |
...
модельдер, дайын тапсыру файлы (нақты LLM-агентпен алынған). Тәуелділіктерді орнатқаннан кейін **желі де, API
...
# 2) Тест кезеңіне агенттің толық rolling-іске қосылуы БӨЛЕК каталогқа (канондық файлдар өзгермейді)
...
python -m scripts.compare_runs --a forecasts --b runs/judge-check   # күтілетіні: сандар сәйкес келеді

# 3) Тесттер және оператор панелі
python -m pytest tests/ -q                            # күтілетіні: 71 passed
streamlit run app.py -- --forecast-dir runs/judge-check              # http://localhost:8501
```

Сіз `[2026-01-31] ok=True -> forecast_t1_..., forecast_t2_...` түріндегі 28 жолды, содан кейін 1344 жолдық жиынтық
файлды көресіз. Панельде — әр түйіннің уақытымен агенттің орындалу графы, P10–P90 жолағымен сағаттық болжам,
...
құрады, екі турбинаның модельдерін іске қосады, тексереді (шектер, толықтық, NaN, flatline), қуат болжамын
...
`mode=openai`, `completed=true`, `fallback=false`; нақты model ID онда жазылмаған.
...
- **LLM құралдарды басқарады және есеп құрастырады**: барлық 28 трассада `mode=openai` және `fallback=false`; нақты model ID сақталмаған. Болжам сандары `--no-llm` режимімен 1e-9 дәлдікпен сәйкес (`scripts/compare_runs`); GPT-6 Luna моделін коммит авторы көрсеткен ([4e6f7ce](docs/INTEGRATION.md#канонические-llm-отчёты-4e6f7ce)).
...
- Өз іске қосуыңыз — `runs/<атау>` ішінде (`--output-dir`): канондық нәтижелер ешқашан қайта жазылмайды; салыстыру — `scripts/compare_runs`.
- 71 автотест: агент графы, LLM адаптерлері, CLI, бағалау хаттамасы, белгілер теңдігі, ауа райы өткізілімдері, оқыту шегі, верификатор, панель.
...
- Бірінші күннің ретроспективті бағалауында MAE номиналдың 15.7 %-ын құрайды; baseline-мен салыстыру және рұқсат етілген мақсаттар маскасы төменде келтірілген.
...
қолданылады. Айдағы орташа болжамдық жүктеме: T1 0.465, T2 0.463 номинал.
...
мақсаттар ([ережелер](docs/DATA.md)). Нормаланған қуаттың MAE: 0.157 = орнатылған қуаттың 15.7 % орташа қатесі.

| Турбина | Көкжиек | Ансамбль MAE | Baseline MAE | RMSE | Сағат-шығарылым |
| --- | --- | --- | --- | --- | --- |
| 1 | 0–24 сағ | **0.1574** | 0.1776 | 0.2332 | 1468 |
| 1 | 24–48 сағ | **0.1729** | 0.1923 | 0.2563 | 1444 |
| 2 | 0–24 сағ | **0.1574** | 0.1792 | 0.2341 | 1438 |
| 2 | 24–48 сағ | **0.1734** | 0.1940 | 0.2581 | 1414 |

**5904 жол** сақталған, бірде-бір шығарылым жоғалмаған; 01.12 күнгі 48 lead-2 жолы алынып тасталды, өйткені
олардың шығарылымы оқыту шегінен бұрын болар еді. P10–P90 қамтуы — номиналды 80 % орнына **65.0 %**: аралықтар
...
| **Баптауға арналған жеке LightGBM** | **0.1647** | **0.1662** |
...
flowchart LR
    ...
    subgraph DayGraph["DayGraph · src/agent/graph.py · LLM және --no-llm үшін бір граф"]
        ...
        C --> D{"validate_forecast<br/>шектер, толықтық,<br/>NaN, flatline"}
...
| [`app.py`](app.py) | Оператор панелі, Streamlit + Plotly | `--forecast-dir` |
...
```bash
docker compose up --build            # құрастыру кезінде модельдерді оқытады, панель http://localhost:8501
...
`forecasts/` каталогы хосттан қосылған. Linux-та таза клоннан контейнерлік репетиция — жеке тексеру
([күй](#-жоба-күйі)).
...
│   ├── report_YYYY-MM-DD.md                  ← операторға агент есебі (LLM)
...
**Тесттер** ([`tests/`](tests/), 71 өтеді): агент графы барлық режимде бірдей, LLM ақауы сақталған түйіннен
...
- **P10–P90 аралықтары фактіні толық қамтымайды** (номинал 80 %-ға қарсы 65 %) — оларды калибрленген 80 % аралығы деп атамау керек.
...
- Linux-та таза клоннан контейнерлік релиз репетициясы — жеке тексеру ([күй](#-жоба-күйі)); Dockerfile-дың
  болуы өзінен-өзі контейнерлік іске қосуды дәлелдемейді.
...
- **Өнімдегі LLM**: әдепкі модель `gpt-6-luna` (OpenAI Chat Completions, tool calling); Claude және NVIDIA NIM адаптерлері. Канондық іске қосудың нақты модель ID-і трассаларда жазылмаған.
...
| 23.09.2026 | Интеграциядан кейінгі канондық LLM іске қосуы: 28/28 трасса `mode=openai`; GPT-6 Luna авторымен көрсетілген, сандар `--no-llm`-мен сәйкес |
| 23.09.2026 | Панель модельді қайта есептеудің орнына сақталған ретроспективті бағалауды көрсетеді; 71 тест |
